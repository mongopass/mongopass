...
def add_password():
    platform_link = simpledialog.askstring(
        "Platform Link", "Enter platform link (e.g., https://github.com):")
    if not platform_link:
        return

    platform_name = urlparse(platform_link).netloc.split('.')[0]
    platform_username = simpledialog.askstring(
        "Platform Username", "Enter username:")
    platform_password = simpledialog.askstring(
        "Platform Password", "Enter password:", show="*")
    tags = simpledialog.askstring(
        "Tags", "Enter comma-separated tags (up to 3):").split(",")[:3]
...
def display_passwords():
    ...
    for pwd in passwords.find({"user_id": current_user_id}):
        password_table.insert("", "end", values=(
            pwd["platform"], pwd["username"], "******", ",".join(pwd["tags"])), tags=('password', pwd["password"]))
